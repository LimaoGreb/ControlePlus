// FAB de lançamento rápido para a aba Casal.
// Mesma UX do QuickAddFab, mas adiciona nos dados compartilhados do casal.
...
export default function CasalQuickFab({ monthIndex }) {
  const { colors } = useTheme();
  const sharedCtx = useSharedData();
  ...
  const reset = () => {
    ...
    setPayment(null); setParcelar(false); setParcelas(2);
  };
  ...
  const add = () => {
    if (!canAdd) return;
    const { addItem, addInstallments } = sharedCtx?.sharedContextValue || {};
    if (!addItem || !addInstallments) return;

    if (parcelar) {
      if (monthIndex >= 11) {
        Alert.alert('Não cabe', 'Dezembro é o último mês — não há meses seguintes para as parcelas.');
        return;
      }
      const added = addInstallments(monthIndex, name.trim() || 'Gasto', valorParcela, parcelas, payment);
      hapticSuccess();
      close();
      const label = name.trim() || 'Gasto';
      if (added < parcelas) {
        Alert.alert('Parcelado ⚠️', `${label}: apenas ${added} de ${parcelas} parcelas cabem até dezembro. As demais foram ignoradas.`);
      } else {
        Alert.alert('Parcelado ✅', `${label} dividido em ${parcelas}x de ${formatBRL(valorParcela)}, adicionado como Gasto Fixo a partir do mês seguinte.`);
      }
    } else {
  ...
  return (
    <>
      ...
      >
        <MaterialCommunityIcons name="heart-plus-outline" size={30} color={contrastText(colors.primary)} />
      </TouchableOpacity>
              ...
              <TouchableWithoutFeedback>
                <Animated.View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border + '80', opacity, transform: [{ scale }] }]}>
                  ...
                  <View style={styles.headerRow}>
                    ...
                    <Text style={[styles.title, { color: colors.text }]}>Gasto do casal</Text>
                  </View>

                  <TextInput
                    value={name}
                    onChangeText={setName}
                    placeholder="No que gastaram? (ex.: Mercado)"
                  ...
                  <TouchableOpacity
                    ...
                    onPress={add}
                  >
                    <Ionicons name={parcelar ? 'card' : 'heart'} size={18} color={contrastText(colors.primary)} />
                    <Text style={[styles.addText, { color: contrastText(colors.primary) }]}>
                      {parcelar ? `Parcelar em ${parcelas}x` : 'Adicionar gasto do casal'}
                    </Text>
                  </TouchableOpacity>
